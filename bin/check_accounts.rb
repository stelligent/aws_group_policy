...
opts = Trollop::options do
  opt :read_only_group, "name of the read only group to assign non-mfa'd users to. Any users that have MFAs will be moved to the full-privileged group.", required: true, type: String
...
def member_of_group? iam, privileged_group, username
  result = false
...
all_users.each do |user|
  if member_of_group? iam, opts[:full_privilege_group], user.user_name
